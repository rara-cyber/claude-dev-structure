...
2.  **Validate Memory Bank State:** Use the memory-bank-synchronizer agent to ensure current patterns and decisions in memory bank are accurate before task generation.
3.  **Analyze PRD:** The AI reads and analyzes the functional requirements, user stories, and other sections of the specified PRD.
4.  **Assess Current State:** Review the existing codebase to understand existing infrastructre, architectural patterns and conventions. Also, identify any existing components or features that already exist and could be relevant to the PRD requirements. Then, identify existing related files, components, and utilities that can be leveraged or need modification.
...
12.  **Update Memory Bank:** Use the memory-bank-synchronizer agent to update CLAUDE-patterns.md with task generation patterns and CLAUDE-decisions.md with any architectural decisions made during task breakdown.
...
The task generation process actively uses the memory-bank-synchronizer agent to:
- **Pre-Generation:** Validate current memory bank accuracy and sync with actual codebase state
- **During Generation:** Apply established patterns and architectural decisions consistently
- **Post-Generation:** Update memory bank with new task patterns and architectural insights

This ensures task lists align with current system architecture and contribute to organizational knowledge.
